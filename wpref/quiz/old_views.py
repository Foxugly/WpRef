...
from .models import QuizTemplate, QuizQuestion, Quiz, QuizAttempt, QuizAnswer
from .old_serializers import (
...
class QuizCreateView(GenericAPIView):
    ...
    def post(self, request, slug):
        quiz = get_object_or_404(QuizTemplate, slug=slug)
...
class QuizSessionView(GenericAPIView):
    ...
    def get(self, request, quiz_id):
        session = get_object_or_404(Quiz, pk=quiz_id)
...
class QuizAttemptView(GenericAPIView):
    ...
    def post(self, request, quiz_id, question_order):
        ...
        if len(request.data.get("selected_option_ids")):
            ...
            if new:
                ...
                # 3) scoring
                earned, max_score = quiz_answer.compute_score()
    ...
    def get(self, request, quiz_id, question_order):
        ...
        show_correct = (
                session.quiz.mode == QuizTemplate.MODE_PRACTICE
...
class QuizViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if request.user.is_staff or getattr(request.user, "is_admin", False):
            ...
            qs = Quiz.objects.filter(user=request.user).order_by("-created_at")
